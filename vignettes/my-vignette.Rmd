...
asked to download the repo and give the directory path at the 5.2 in oreder to 
...
*[aggiungi required/optional/etc]*

1. Load the cohort minimal metadata in the form of PED file;
2. Load the markers file (i.e. genomic location of each marker, SNP in case of array, interval/bin
  in case of NGS data);
3. Read and pre-process the results for all samples from the individual algorithms/pipelines, in particular 
  adjacent calls with equal genotype (i.e. deletions/duplications) can be merged if closer than 
  user-specified distances;
4. Read and pre-process the "raw data" (in particular the log2R for each marker for all samples);
5. Explore the sample's summary statistics, identify groups of unwanted events (e.g. smaller 
  then 5 kbp or consisting of less than 5 points) and possible over-segmented samples;
6. Filter the results based on several parameters and blacklist (both provided by the package and 
...
7. Detect CNVs inheritance pattern, if families structure is given;
8. Visually inspect the good de novo CNVs candidate raw data (LRR/log2R in particular);
9. Merge the various methods results in a single object, keep only the call made by N (one or more)
  algorithms/pipelines if desired;
1. Annotate genomic locus of the calls and search for CNVs in known disease-linked loci (e.g. 
  in studies on ASD or schizophrenia). Annotate genic content of selected CNVs;
2. Compute CNV regions (CNVRs); 
3. Extract high confidence rare events based on CNVRs; 
4. Export desired data as BED (e.g. for being viewed in IGV) or VCF (e.g. for being processed with Annovar)
  , as well as TSV.
...
```{r blacklists, eval=FALSE, echo=-5}
...
```{r cnvrs, eval=FALSE}
cnvrs <- cnvrs_create(cnvs = ipq_filtered, chr_arms = hg19_chr_arms)
```
...
Examples using `data.table` syntax are shown in the "Data Export" section.
...
# locus 
ipq_denovo_locus <- genomic_locus(DT_in = ipq_denovo, keep_str_end = FALSE)
...
`GRanges` will be implemented soon.    
...
ipq_filtered_GRanges
```

Conversion to `GRangesList` (e.g. for integration with `CNVranger` package) will 
be implemented soon. 
...
# lost when combining more results)
...
common_cnvrs <- cnvrs[[1]][freq > nrow(cohort)*0.5, ]
rare_cvns <- cnvrs[[2]][!cnvr %in% common_cnvrs$r_ID]
```
